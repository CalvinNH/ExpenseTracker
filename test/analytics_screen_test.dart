...
void main() {
  ...
  testWidgets(
    'Analytics Screen displays visual charts and tab switcher correctly',
    (WidgetTester tester) async {
      final db = AppDatabase.instance;

      // Seed database with multiple accounts of different types
      final hdfcCcId = await db.createAccount(
        Account(bankName: 'HDFC Platinum CC', currentBalance: -5000.0),
      );

      final hdfcSavingsId = await db.createAccount(
        Account(bankName: 'HDFC Savings', currentBalance: 50000.0),
      );
  ...
  testWidgets(
    'Analytics Screen filters transactions dynamically based on period selection',
    (WidgetTester tester) async {
      final db = AppDatabase.instance;
